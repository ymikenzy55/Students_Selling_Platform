...
export default function MessagesPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-2 sm:px-4 lg:px-8 py-3 sm:py-4 h-[calc(100vh-80px)]">
        <div className="mb-2 sm:mb-3">
          <h1 className="text-xl sm:text-2xl font-extrabold text-gray-900 mb-0.5 sm:mb-1">Messages</h1>
          <p className="text-xs sm:text-sm text-gray-600">
            {totalUnread > 0 ? `${totalUnread} unread message${totalUnread > 1 ? 's' : ''}` : 'All caught up!'}
          </p>
        </div>

        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden h-[calc(100%-50px)] sm:h-[calc(100%-70px)] flex">
          <div className="grid grid-cols-1 md:grid-cols-3 h-full w-full min-h-0">
            {/* Conversations List - Hidden on mobile when chat is selected */}
            <div className={`border-r border-gray-200 flex flex-col h-full min-h-0 ${selectedConversation ? 'hidden md:flex' : 'flex'}`}>
              {/* Search */}
            ...
            {selectedConversation ? (
              <div className={`md:col-span-2 flex flex-col h-full min-h-0 ${selectedConversation ? 'flex' : 'hidden md:flex'}`}>
                {/* Chat Header */}
                <div className="p-2.5 sm:p-3 border-b border-gray-200 flex items-center justify-between flex-shrink-0">
                  <div className="flex items-center gap-2 sm:gap-2.5">
                    {/* Back button for mobile */}
                    <button
                      onClick={() => setSelectedConversation(null)}
                      className="md:hidden p-1.5 hover:bg-gray-100 rounded-lg transition-colors cursor-pointer mr-1"
                    >
                ...
                {/* Messages */}
                <div className="flex-1 overflow-y-auto p-2.5 sm:p-4 space-y-2 sm:space-y-3 bg-gray-50 min-h-0">
                  {messages.map((message, index) => {
                    ...
                    return (
                      <div
                        key={message.id}
                        className={`flex gap-1.5 sm:gap-2 ${isCurrentUser ? 'flex-row-reverse' : 'flex-row'}`}
                      >
                        {/* Avatar */}
                        ...
                        <div className={`flex flex-col max-w-[75%] sm:max-w-[70%] ${isCurrentUser ? 'items-end' : 'items-start'}`}>
                          <div
                            className={`px-2.5 py-1.5 sm:px-3 sm:py-2 rounded-2xl ${
                              isCurrentUser
                ...
                {/* Message Input */}
                <form onSubmit={handleSendMessage} className="p-2 sm:p-3 border-t border-gray-200 bg-white flex-shrink-0">
                  <div className="flex items-center gap-1.5 sm:gap-2">
                    ...
                    <div className="flex-1 relative">
                      <input
                        ...
                        className="w-full px-2.5 sm:px-3 py-2 text-xs sm:text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent text-gray-900 bg-white pr-8 sm:pr-10"
                      />
                      <button
                        type="button"
                        className="absolute right-1.5 sm:right-2 top-1/2 -translate-y-1/2 p-1 hover:bg-gray-100 rounded-lg transition-colors cursor-pointer"
                        title="Add emoji"
                      >
                        <Smile className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-gray-500" />
                      </button>
                    </div>
                    <button
                      type="submit"
                      disabled={!newMessage.trim()}
                      className="p-2 sm:p-2.5 bg-gradient-to-r from-purple-600 to-purple-500 text-white rounded-lg hover:from-purple-700 hover:to-purple-600 transition-all disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
                      title="Send message"
                    >
